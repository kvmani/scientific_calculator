...
def security_headers(response: Response) -> Response:
    ...
    response.headers.setdefault("Content-Security-Policy", "default-src 'self'; style-src 'self' 'unsafe-inline'; script-src 'self' 'unsafe-inline'; object-src 'none'; base-uri 'self'; frame-ancestors 'self'")
...
@app.get("/api/health")
def health(): return jsonify({"status": "ok", "tool_id": "scientific-calculator", "version": VERSION})
...
def main() -> None:  # pragma: no cover
    ...
    parser = argparse.ArgumentParser(description="Scientific Calculator service")
